...
public class NodeGenerator : MonoBehaviour
{
    ...
    public void MoveNodes(Vector2 targetPos)
    {
        ...
        {
            StartCoroutine(MovePlayer(nodes[i], previousNodePositions[i - 1]));
        }
    }

    private IEnumerator MovePlayer(Transform node, Vector2 direction)
    {
        Vector2 origPos = node.position;
        ...
        {
            node.position = Vector2.Lerp(origPos, targetPos, (elapsedTime / timeToMove));
    ...
    public void CreateNewNode(Vector2 lastFacedDirection, LayerMask groundLayer)
    {
        ...
        CreateAINode((Vector2)lastNodePosition + nodeDirection);
